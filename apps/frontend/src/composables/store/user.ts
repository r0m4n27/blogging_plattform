...
} from "@blog/frontend/api/auth";
import { localStorageKeys } from "@blog/frontend/config/localStorage";
...
import { FetchError } from "@blog/frontend/lib/fetch";
...
export interface UserState {
  ...
  fetchWithToken<T>(fetcher: (token: string) => Promise<T>): Promise<T | undefined>;
  fetchWithToken<T>(
...
export const useUser = defineStore<string, UserState>(piniaKeysConfig.user, () => {
